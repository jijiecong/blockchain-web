package com.meiren.common.core.method.annotation;

import com.meiren.common.core.bind.annotation.FormModel;
import com.meiren.common.core.utils.MapWapper;
 ...
 * https://github.com/zhangkaitao/es/blob/master/common/src/main/java/com/sishuok/es/common/web/bind/method/annotation/FormModelMethodArgumentResolver.java
 * @since 3.1
 */
public class FormModelMethodArgumentResolver extends BaseMethodArgumentResolver {
	 ...
	 *             next method parameter is not of type
	 *             {@link Errors}.
	 * @throws Exception
	...
	public final Object resolveArgument(MethodParameter parameter,
		...
		if (target != null) {
			bindRequestParameters(mavContainer, binderFactory, binder, request,
					parameter);
	 ...
	 * {@link ServletRequestDataBinder} before
	 * binding.
	 *
	 * @throws Exception
	...
	protected void bindRequestParameters(ModelAndViewContainer mavContainer,
		...
		if (Collection.class.isAssignableFrom(targetType)) {// bind collection
				...
				if (isSimpleComponent(prefixName)) { // bind simple type
					...
					if (!matcher.matches()) { // 处理如 array=1&array=2的情况
						for (Object value : paramValues.values()) {
							targetList.add(simpleBinder.convertIfNecessary(
									value, componentType));
						}
						...
						targetList.set(
								index,
								simpleBinder.convertIfNecessary(
										paramValues.values(), componentType));
					}
				} else { // 处理如
					...
					if (!matcher.matches()) {
						throw new IllegalArgumentException(
								"bind collection error, need integer index, key:"
										+ key);
					}
					int index = Integer.valueOf(matcher.group(1));
					if (targetList.size() <= index) {
					...
					WebDataBinder componentBinder = binderFactory.createBinder(
					...
					if (component != null) {
						...
						if (componentBinder.getBindingResult().hasErrors()) {
							if (isBindExceptionRequired(componentBinder,
									parameter)) {
								throw new BindException(
										componentBinder.getBindingResult());
							}
						}
			...
			if (type instanceof ParameterizedType) {
				keyType = (Class<?>) ((ParameterizedType) type)
						.getActualTypeArguments()[0];
				valueType = (Class<?>) ((ParameterizedType) type)
			...
			for (Object key : servletRequest.getParameterMap().keySet()) {
				...
				Object keyValue = simpleBinder.convertIfNecessary(
						getMapKey(prefixName), keyType);
				...
				if (isSimpleComponent(prefixName)) { // bind simple type
					...
					for (Object value : paramValues.values()) {
						target.put(keyValue, simpleBinder.convertIfNecessary(
								value, valueType));
					}
				} else {
	...
	private void growCollectionIfNecessary(final Collection collection,
			final int index) {
	...
	private ServletRequest prepareServletRequest(Object target,
		...
		if (multipartRequest != null) {
			...
			for (MultipartFile file : multipartRequest.getFileMap().values()) {
				mockMultipartRequest.addFile(new MultipartFileWrapper(
	...
	protected void validateObject(WebDataBinder binder,
	...
	/**
	 * Whether to raise a {@link BindException}
